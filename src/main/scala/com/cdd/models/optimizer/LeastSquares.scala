...
import com.cdd.models.datatable.{DenseVector, VectorBase}
import com.cdd.models.utils.HasLogging

// based on code from org.apache.spark.ml.optim.WeightedLeastSquares.scala
...
class WeightedLeastSquares(
    ...
    private val solverType: WeightedLeastSquares.Solver = WeightedLeastSquares.Auto,
    val maxIter: Int = 100,
    val tol: Double = 1e-6) extends HasLogging with Serializable {
  import WeightedLeastSquares._

  require(regParam >= 0.0, s"regParam cannot be negative: $regParam")
  if (regParam == 0.0) {
    logger.warn("regParam is zero, which might cause numerical instability and overfitting.")
  }
  ...
  /**
   * Creates a [[WeightedLeastSquaresModel]]
   */
  def fit(labels: Vector[Double], features: Vector[VectorBase], weights: Option[Vector[Double]]): WeightedLeastSquaresModel = {
    val summary = new Summary(labels, features, weights)
    summary.validate()
    logger.info(s"Number of instances: ${summary.count}.")
    ...
    if (rawBStd == 0) {
      if (fitIntercept || rawBBar == 0.0) {
        ...
        val coefficients = new DenseVector(Array.ofDim[Double](numFeatures))
        ...
        logger.warn(s"The standard deviation of the label is zero. Consider setting " +
          s"fitIntercept=true.")
      }
    }
      ...
      case cholesky: CholeskySolver =>
        try {
          ...
          case _: SingularMatrixException if solverType == WeightedLeastSquares.Auto =>
            logger.warn("Cholesky solver failed due to singular covariance matrix. " +
...
object WeightedLeastSquares {
  ...
  /**
   * Class provide necessary summary statistics for solving [[WeightedLeastSquares]].
   */
  @SerialVersionUID(1000L)
  private class Summary(labels: Vector[Double], features: Vector[VectorBase], weights: Option[Vector[Double]]) extends Serializable {
     val k:Int =  features(0).length()
    ...
    var count: Int = features.length
     val triK: Int = k * (k + 1) / 2
     var wSum: Double = 0.0
    private var wwSum: Double = 0.0
    private var bSum: Double = 0.0
    private var bbSum: Double = 0.0
    private val aSum: DenseVector = new DenseVector(Array.ofDim[Double](k))
    private val abSum: DenseVector = new DenseVector(Array.ofDim[Double](k))
    private val aaSum: DenseVector = new DenseVector(Array.ofDim[Double](triK))


    labels.zip(features).zipWithIndex.foreach { case ((l, f), i) =>
      assert(f.length() == k, s"Dimension mismatch. Expect vectors of size $k but got ${f.length()}.")
      val w = weights match {
