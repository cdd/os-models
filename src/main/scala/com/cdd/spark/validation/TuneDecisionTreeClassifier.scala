...
object TuneDecisionTreeClassifier extends App {


  val tuner = new TuneDecisionTreeClassifier(false, args.toList)
  tuner.tune()
}

class TuneDecisionTreeClassifier(removeExistingResults: Boolean = false, names: List[String] = List()) extends Tuner with HasLogging {


  override def tune(): Unit = {

    val dt = new DecisionTreeClassifier().setFeaturesCol("uniqueFeatures").setLabelCol("label").setSeed(1234L)
    ...
    val (resultsFile, currentResults) = SparkTestSystems.prepareResultFile("tune_dtc.parquet")
    ...
    val func = (name: String, ds: DataFrame) => {
      ...
      val results: Seq[(String, String, String, Int, Int, Double, Double, Double, Double)] = combined
        .map { case ((p, auc), m) =>
          (name, "DTC", p(filter.inputCol), p(dt.maxDepth), p(dt.maxBins), auc, m(0), m(1), m(2))
        }
      val resultsDf = ds.sparkSession.createDataFrame(Configuration.sparkContext.parallelize(results, 1))
        .toDF("name", "method", "feature", "depth", "bins", "auc", "f1", "f2", "f3")
