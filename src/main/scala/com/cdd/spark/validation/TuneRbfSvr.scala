...
object TuneRbfSvr extends App {
  ...
  val tuner = new TuneRbfSvr(false, args.toList)
  tuner.tune()
}
...
/**
  ...
  */
class TuneRbfSvr(removeExistingResults: Boolean = true, names: List[String] = List()) extends Tuner with HasLogging{
  ...
  def tune(): Unit = {
    ...
    val paramGrid = new ParamGridBuilder()
      ...
      .addGrid(svr.C, Array(0.01, 0.1, 1, 10, 100, 1000))
      .addGrid(svr.gamma, Array(0.0001, 0.001, 0.01, 0.1, 1))
      .build()
    ...
    val (resultsFile, currentResults) = SparkTestSystems.prepareResultFile("tune_rbf_svr_fp.parquet")
    ...
    val func = (name: String, ds: DataFrame) => {
      ...
      val results: Seq[(String, String, String, Int, Double, Double, Double, Double, Double, Double)] = combined
        .map { case ((p, rmse), m) =>
          (name, "SVR_RBF", p(folder.inputCol), p(folder.foldSize), p(svr.C), p(svr.gamma), rmse, m(0), m(1), m(2))
        }
      val resultsDf = ds.sparkSession.createDataFrame(Configuration.sparkContext.parallelize(results, 1))
        .toDF("name", "method", "feature", "nfolds", "C", "gamma", "rmse", "f1", "f2", "f3")
