...
object TuneLogisiticRegressionClassifier extends App {


  val tuner = new TuneLogisiticRegressionClassifier(false)
  tuner.tune()
}

class TuneLogisiticRegressionClassifier(removeExistingResults: Boolean = false, names: List[String] = List(), scale:Boolean = true) extends Tuner with HasLogging{
  ...
  override def tune(): Unit = {
    ...
    val pipeline = new Pipeline().setStages(Array(filter, folder, lr))
    ...
    val func = (name: String, ds: DataFrame) => {
      ...
      val results: Seq[(String, String, String, Int, Double, Int, Double, Double, Double, Double, Double)] = combined
        .map { case ((p, auc), m) =>
          (name, methodName, p(filter.inputCol), p(folder.foldSize), p(lr.elasticNetParam), p(lr.maxIter), p(lr.regParam), auc, m(0), m(1), m(2))
        }
      val resultsDf = ds.sparkSession.createDataFrame(Configuration.sparkContext.parallelize(results, 1))
        .toDF("name", "method", "feature", "nfolds", "elasticNetParam", "maxIter", "reg", "auc", "f1", "f2", "f3")
