...
public class SVM<T> implements OnlineClassifier<T>, SoftClassifier<T>, Serializable {
    ...
    /**
     * GJ added this: maxIter
     * Maximum number of smo iterations in final optimization
     */
    ...
    public static class Trainer<T> extends ClassifierTrainer<T> {
        ...
        public SVM<T> train(T[] x, int[] y, double[] weight) {
            SVM<T> svm = null;
            if (k == 2) {
                svm = new SVM<>(kernel, Cp, Cn, -1);
            } else {
                if (this.weight == null) {
                    svm = new SVM<>(kernel, Cp, k, strategy, -1);
                } else {
        ...
        void finish(double epsgr) {
            ...
            for (int count = 1; smo(null, null, epsgr); count++) {
                ...
                if (maxIter > 0 && count >= maxIter) {
                    logger.warn("exceeded maximum number "+maxIter+" of iterations");
                    break;
                }
            }
     ...
     * @param C      the soft margin penalty parameter.
     */
    public SVM(MercerKernel<T> kernel, double C, int maxIter) {
        this(kernel, C, C, maxIter);
    }
     ...
     * @param Cn     the soft margin penalty parameter for negative instances.
     */
    public SVM(MercerKernel<T> kernel, double Cp, double Cn, int maxIter) {
        if (Cp < 0.0) {
            throw new IllegalArgumentException("Invalid postive instance soft margin penalty: " + Cp);
        }
     ...
     * @param k      the number of classes.
     */
    public SVM(MercerKernel<T> kernel, double C, int k, Multiclass strategy, int maxIter) {
        if (C < 0.0) {
            throw new IllegalArgumentException("Invalid soft margin penalty: " + C);
        }
