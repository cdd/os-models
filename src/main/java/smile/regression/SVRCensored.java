...
// based on Smile SVR with extensions for censored data.
...
import com.cdd.models.utils.CensoredValue;
 ...
 * @author Haifeng Li
 */
public class SVRCensored<T> implements Serializable {
    ...
    /**
     * Maximum number of iterations- GJ extended smile SVR class
     */
    private double maxIter = -1;
    ...
    class SupportVector implements Serializable {
        ...
        private SupportVector(T x, CensoredValue y, double c, double k) {
            this.x = x;
            this.y = y;
            C = c;
            this.k = k;
    ...
    public static class Trainer<T> {
        ...
        public SVRCensored<T> train(T[] x, CensoredValue[] y) {
            SVRCensored<T> svr = new SVRCensored<>(x, y, kernel, eps, C, tol);
            return svr;
        }
    }
     ...
     * @param C      the soft margin penalty parameter.
     */
    public SVRCensored(T[] x, CensoredValue[] y, MercerKernel<T> kernel, double eps, double C) {
     ...
     * @param C      the soft margin penalty parameter.
     */
    public SVRCensored(T[] x, CensoredValue[] y, double[] weight, MercerKernel<T> kernel, double eps, double C) {
        this(x, y, weight, kernel, eps, C, 1E-3, -1);
    }
     ...
     *
     * @param x      training instances.
     * @param y      response variable with censored bounds.
     * @param kernel the kernel function.
     * @param eps    the loss function error threshold.
     * @param C      the soft margin penalty parameter.
     * @param tol    the tolerance of convergence test.
     */
    public SVRCensored(T[] x, CensoredValue[] y, MercerKernel<T> kernel, double eps, double C, double tol) {
        this(x, y, null, kernel, eps, C, tol, -1);
    }
     ...
     * @param tol    the tolerance of convergence test.
     */
    public SVRCensored(T[] x, CensoredValue[] y, double[] weight, MercerKernel<T> kernel, double eps, double C, double tol, int maxIter) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            SupportVector v = new SupportVector(x[i], y[i], w*C, kernel.k(x[i], x[i]));
            if (y[i].getModifier() != Modifier.GREATER_THAN)
                v.g[0] = eps + y[i].getValue();
            else
                v.g[0] = Double.NaN;
            if (y[i].getModifier() != Modifier.LESS_THAN)
                v.g[1] = eps - y[i].getValue();
            else
                v.g[1] = Double.NaN;
            sv.add(v);
        }
        ...
        for (int count = 1; smo(tol); count++) {
            if (maxIter > 0 && count >= maxIter) {
                logger.info("SVR maximum iteration count {} hit", maxIter);
                break;
            }
            if (count % phase == 0) {
    ...
    private boolean smo(double epsgr) {
        ...
        for (SupportVector v : sv) {
            ...
            if (v.y.getModifier() != Modifier.GREATER_THAN) {
                double gj = -v.g[0];
            ...
            if (v.y.getModifier() != Modifier.LESS_THAN) {
                double gj = v.g[1];
        ...
        if (ii == 0)
            assert(i.y.getModifier() != Modifier.GREATER_THAN);
        else
            assert(i.y.getModifier() != Modifier.LESS_THAN);
        if (jj == 0)
            assert(j.y.getModifier() != Modifier.GREATER_THAN);
        else
            assert(j.y.getModifier() != Modifier.LESS_THAN);
