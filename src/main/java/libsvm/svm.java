...
/*
GJ modified Solve routines to take a max_iter parameter for maximum number iterations
 */
...
class Solver {
	...
	void Solve(int l, QMatrix Q, double[] p_, byte[] y_,
		   double[] alpha_, double Cp, double Cn, double eps, SolutionInfo si, int shrinking, int max_iter)
	{
		this.l = l;
		this.Q = Q;
		QD = Q.get_QD();
		...
		int iter = 0;
		if (max_iter == -1)
			max_iter = Math.max(10000000, l>Integer.MAX_VALUE/100 ? Integer.MAX_VALUE : 100*l);
...
final class Solver_NU extends Solver
{
	...
	{
		this.si = si;
		super.Solve(l,Q,p,y,alpha,Cp,Cn,eps,si,shrinking, max_iter);
	}
