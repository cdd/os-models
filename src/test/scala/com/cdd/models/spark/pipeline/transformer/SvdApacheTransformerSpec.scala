...
import com.cdd.spark.pipeline.transformer.SvdApacheTransformer
...
/**
  ...
  */
class SvdApacheTransformerSpec extends FunSpec with Matchers {

  describe("Performing SVD on sparse fingerprints using apache math svd") {

    // var df = ModelContext.sparkSession.read.parquet(Util.getProjectFilePath("data/continuous/Chagas.parquet").getAbsolutePath)
    ...
    val svdTransform = new SvdApacheTransformer().setInputCol("rdkit").setK(200)
